...
filter_df <- covid_daily_df %>% filter(date %in% c(
  "2021-1-01", "2021-2-01", "2021-3-01", "2021-4-01", "2021-5-01",
...
server <- function(input, output) {
  ...
  output$covid_tracker <- renderPlotly({
    ...
    covid_tracker <- ggplot(
      data = filtered_daily_df,
      aes(x = date, y = daily_new_cases)
    ) +
      geom_line() +
      labs(
        title = "COVID-19 Cases Tracker", x = "Date",
        y = "Daily New Cases"
      ) +
      theme_minimal()
  ...
  output$covid_country_plot <- renderPlotly({
    filter_df <- filter_df %>% filter(country %in% c(input$checkbox_selection))

    covid_plot_2 <- ggplot(data = filter_df) +
      geom_point(aes(x = date, y = daily_new_cases, color = country)) +
      scale_x_discrete(labels = c(
        "2021-1-01" = "Jan 1", "2021-2-01" = "Feb 1", "2021-3-01" = "Mar 1",
      ...
      labs(
        title = "Daily New Cases in World's Top Five Economies 2021", x = "Months (1st)",
        y = "Daily New Cases", color = "Countries"
      ) +
